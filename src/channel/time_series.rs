...
impl<T> TSAnyBuffer<T> {
    pub fn new(
        ...
        match (bounded, dispatch) {
            ...
            // (Some(bounded), false) => {
            //     TSAnyBuffer::BoundedBuffer(TSBoundedBuffer::<T>::new(bounded))
            // }
            // (None, true) => {
            //     TSAnyBuffer::UnboundedDispatchBuffer(TSUnboundedDispatchBuffer::<T>::new())
            // }
            // (Some(bounded), true) => {
            //     TSAnyBuffer::BoundedDispatchBuffer(TSBoundedDispatchBuffer::<T>::new(bounded))
            // }
            _ => panic!(),
        }
    }
